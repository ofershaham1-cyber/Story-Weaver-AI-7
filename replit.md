...
- **Settings schema versioning**: `use-settings.ts` carries a `settingsVersion` field (current = 1). The `load()` migration re-applies select defaults for any payload below the current version — used to recover users from a prior bad default of `ttsTranslationMode: "off"` so picking View languages now auto-plays them.
